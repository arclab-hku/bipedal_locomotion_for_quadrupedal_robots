...
class Bipedaldog(LeggedRobot):
    ...
    def _get_noise_scale_vec(self, cfg):
        ...
        if self.cfg.env.train_type == "standard":
            ...
            # noise_vec[3:6] = noise_scales.gravity * noise_level
            # noise_vec[6:9] = 0.  # commands
            # noise_vec[9:21] = noise_scales.dof_vel * noise_level * self.obs_scales.dof_vel
            # noise_vec[21:33] = 0.
    ...
    def compute_observations(self):
        ...
        # self.feet_normal_force = self.contact_forces[:, self.feet_indices, 2] + torch.tensor([[0, 1e-6, 0, 1e-6]] * self.num_envs, device=self.device)
        self.feet_normal_force = self.contact_forces[:, self.feet_indices, 2] + torch.tensor([[0, 0, 0, 1e-6]] * self.num_envs, device=self.device)
    ...
    def _push_robots_debug(self, force_norm, force_direction):
    ...
    def _reward_l_contact_force(self):
        foot_forces = torch.norm(self.contact_forces[:, self.feet_indices, :], dim=-1)
        # print("-------------------------")
        # print(self.contact_forces[:, self.feet_indices, :])
        # print(foot_forces[:, 0])
        l_force = foot_forces[:, 0] + foot_forces[:, 2]
        return l_force

    def _reward_ri_contact_force(self):
        foot_forces = torch.norm(self.contact_forces[:, self.feet_indices, :], dim=-1)
        # print("-------------------------")
        # print(self.contact_forces[:, self.feet_indices, :])
        # print(foot_forces[:, 0])
        ri_force = foot_forces[:, 1] + foot_forces[:, 3]
        return ri_force
    ...
    def _reward_inv_pendulum(self):
        ...
        # self.bipedal_data["angle"] += self.pen_ang.clone().detach().cpu().numpy().tolist()
        # np.savetxt("angle.txt", np.array(self.bipedal_data["angle"]))
    ...
    def _reward_inv_pendulum_acc(self):
        ...
        # self.bipedal_data["angular_acc"] += self.pen_ang_acc.clone().detach().cpu().numpy().tolist()
        # np.savetxt("angular_acc.txt", np.array(self.bipedal_data["angular_acc"]))
